        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-auto float-right ml-auto">
                    <a href="#" class="btn add-btn" data-toggle="modal" data-target="#create_project" title=" Create Project"><i class="fa fa-plus"></i></a>
                    ...
                    <div class="card-body">
                        <div class="dropdown dropdown-action profile-action">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item" href="{{ url('edit-projects/'.$project->id) }}"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                <a class="dropdown-item btn_project_delete" href="#" data="{{ $project->id }}"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                            </div>
                        </div>
                        ...
                        <small class="block text-ellipsis m-b-15">
                            ...
                            @php
                            $open=App\Models\Tasks::where('project_id',$project->id)->where('status',2)->count();
                            $pending=App\Models\Tasks::where('project_id',$project->id)->where('status',0)->count();
                            $complete=App\Models\Tasks::where('project_id',$project->id)->where('status',1)->count();
                            @endphp

                            <span class="text-xs"> <strong>{{$open}} </strong> </span> <span class="badge-primary">open </span>
                            <span class="text-xs"> <strong>{{$pending}} </strong></span> <span class="badge-danger"> pending</span>
                            <span class="text-xs"> <strong>{{$complete}} </strong></span> <span class="badge-success"> completed</span>
                            ...
                            <ul class="team-members">
                                <li>
                                    <a href="#" data-toggle="tooltip" title="{{$project->name}}">
                                        ...
                                        <img alt="" src="{{asset('storage/app/public/uploads/staff-images/').'/'.$project->image}}">
                                    </a>
                                </li>
                            </ul>
                        </div>
                        ...
                        $doneTask= APP\Models\Project::join('tasks','tasks.project_id','=','projects.id')->where('tasks.project_id',$project->id)->where('tasks.status',1)->get();
                        ...
                        <p class="m-b-5">Progress <span class="text-success float-right"> {{round($progress)}}%</span></p>
                        <div class="progress progress-xs mb-0">
                            <div class="progress-bar bg-success" role="progressbar" data-toggle="tooltip" title="Progress" style="width:{{round($progress)}}%"></div>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{url('projects')}}" class="needs-validation" novalidate enctype="multipart/form-data" id="ProjectForm">
                        @csrf
                        <div class="row">
                            <div class="col-sm-12">
