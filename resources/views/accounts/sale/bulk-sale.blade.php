        ...
        <div class="content container-fluid">
            <div class="page-header my-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title bold-heading">Bulk Sale</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{url('/')}}">Dashboard</a></li>
                            <li class="breadcrumb-item active">Bulk Sale</li>
                        </ul>
                    </div>
                    <div class="col-auto float-right ml-auto">
                        <a href="{{url('/sale-report')}}" class="btn add-btn" title="Sale List"><i class="fa fa-list" aria-hidden="true"></i></a>
                    </div>
                </div>
            </div>
            <div class="card">
                <form action="{{url('bulk-sale')}}" method="post" id="purchaseForm" class="needs-validation" novalidate>
                        @csrf
                        <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <select name="client_id" class="form-control selectpicker" data-container="body" data-live-search="true" title="Choose Client" required>
                            ...
                            <div class="col-md-2 balance-section">
                                <input type="text" class="form-control" name="balance" placeholder="Available Balance" readonly>
                            </div>
...
{{--                                    <th>Price <span class="text-danger">*</span></th>--}}
                                    ...
                                    <th>Sub Total <span class="text-danger"></span></th>
                                    <th></th>
                                </tr>
                                </thead>
...
{{--                                    <td><input type="number" class="form-control price" placeholder="0.00" name="price[]" required readonly></td>--}}
                                    ...
                                    <td><input type="number" class="form-control sub-total" placeholder="0.00" name="total[]" required readonly></td>
                                ...
                                <tr>
                                    ...
                                    <td><input type="number" class="form-control" placeholder="0.00" name="grand_total" readonly></td>
                                </tr>
                                </tbody>
                            </table>
...
{{--                <td><input type="number" class="form-control price" placeholder="0.00" name="price[]" required readonly></td>--}}
                ...
                {{--
                <td><input type="number" class="form-control discount" placeholder="0.00" name="discount[]" required></td>
                --}}
                <td><input type="number" class="form-control sub-total" placeholder="0.00" name="total[]" required></td>
...
//getting inventroy stock
                ...
                $.ajax({
                    ...
                    data: {inventory_type: inventory_type,sale_type:'bulk'},
                    async: false,
                    dataType: 'json',
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            for (i = 0; i < data.length; i++) {
                                html += '<option value="' + data[i].id + '">' + data[i].item + '</option>';
                            }
                        } else {
                            var html = '<option value="">Choose Item</option>';
            ...
            $('#tblPurchase').on('keyup','.qty', function() {
                ...
                var item_id = $currentRow.find('.item-id').val();
                var inventory_type=$('select[name=inventory_type]').val();
                if(qty < 1){
                    $currentRow.find('.sub-total').val(0);
                }

                $.ajax({
                    type: 'ajax',
                    method: 'get',
                    url: '{{url("/getAvailStockAndSubTotal")}}',
                    data: {item_id: item_id,qty:qty,inventory_type:inventory_type},
                    async: false,
                    dataType: 'json',
                    success: function(data) {
                        if(data.error){
                            ...
                            var p='<span style="color:red">Stock Low</span>';
                        ...
                        if(data.subTotal){
                            $currentRow.find('.sub-total').val(data.subTotal);
                        }
                    },
