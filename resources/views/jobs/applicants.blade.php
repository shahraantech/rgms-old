                    ...
                    <div class="card-body">
                        <form action="{{ url('job-applicants') }}" method="post" id="searchForm">
                            ...
                            <div class="row filter-row">
                                ...
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-primary search_appli"><i class="fa fa-search"></i></button>
                                </div>
        ...
        $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            html += '<tr>' +
                                ...
                                '<i class="fa fa-dot-circle-o text-success"></i>' + data[i].status +
                                '</a>' +
                                '</div>' +
                                '</td>' +
                                ' <td><a href="{{ url('/download-resume') }}/' + data[i].id +
                                '" class="btn btn-sm btn-primary btnDownload" title="Download"><i class="fa fa-download"></i> </a></td>' +
                                '<td class="text-right">' +
                                '<div class="dropdown dropdown-action">' +
                                '<a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>' +
                                '<div class="dropdown-menu dropdown-menu-right">' +
                                '<a class="dropdown-item btnInterview" href="javascript:" data="' +
                                data[i].id +
                                '"><i class="fa fa-clock-o m-r-5 text-success" ></i> Schedule Interview</a>' +
                                '<a class="dropdown-item btn-status" href="javascript:" data="' + data[
                                    i].id +
                                '" status="Rejected"><i class="fa fa-clock-o m-r-5 text-danger" ></i> Rejected</a>' +
                                '<a class="dropdown-item btn-status" href="javascript:" data="' + data[
                                    i].id +
                                '"  status="Short List"><i class="fa fa-clock-o m-r-5 text-success"></i> Short List</a>' +
                                '<a class="dropdown-item btn-status" href="javascript:" data="' + data[
                                    i].id +
                                '"  status="hired"><i class="fa fa-clock-o m-r-5 text-success"></i> Hired</a>' +
                                '</div>' +
                                ' </div>' +
                                '</td>' +
                                '</tr>';
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.errors) {
                            ...
                            toastr.error(data.errors['interview_time'], data.errors[
                                'interview_date']);
                        }
            ...
            $('#searchForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    processData: false,
                    beforeSend: function() {
                        $('.search_appli').text('Searching...');
                        $(".search_appli").prop("disabled", true);
                    },
                    success: function(data) {
                        var html = '';
                        var i;
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            html += '<tr>' +
                                ...
                                ' <td>' + data[i].email + '</td>' +
                                '<td>' + data[i].phone + '</td>' +
                                ' <td>' + moment(data[i].created_at).format('DD-MMM-YYYY') +
                                '</td>' +
                                ' <td class="text-center">' +
                                '<div class="dropdown action-label">' +
                                '<a class="btn btn-white btn-sm btn-rounded dropdown-toggle" href="#" data-toggle="dropdown" aria-expanded="false">' +
                                '<i class="fa fa-dot-circle-o text-success"></i>' + data[i]
                                .status + '</a>' +
                                ...
                                '</div>' +
                                '</td>' +
                                ' <td><a href="{{ url(' / download - resume ') }}/' + data[i]
                                .id +
                                '" class="btn btn-sm btn-primary btnDownload"><i class="fa fa-download"></i> Download</a></td>' +
                                '<td class="text-right">' +
                                '<div class="dropdown dropdown-action">' +
                                '<a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>' +
                                '<div class="dropdown-menu dropdown-menu-right">' +
                                ...
                                data[i].id +
                                '"><i class="fa fa-clock-o m-r-5 text-success" ></i> Schedule Interview</a>' +
                                '<a class="dropdown-item btn-status" href="javascript:" data="' +
                                data[i].id +
                                '" status="Rejected"><i class="fa fa-clock-o m-r-5 text-danger" ></i> Rejected</a>' +
                                '<a class="dropdown-item btn-status" href="javascript:" data="' +
                                data[i].id +
                        ...
                        $('#showApplicantsList').html(html);
                        $('.search_appli').html('<i class="fa fa-search"></i>');
                        $(".search_appli").prop("disabled", false);
