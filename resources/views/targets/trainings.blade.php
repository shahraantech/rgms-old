                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <ul class="team-members">
                                                            ...
                                                            @endphp
                                                            <li>
                                                                <a href="#" title="{{ $train->name }}"
                                                                    data-toggle="tooltip">
                                                                    <img alt="" class="target-img"
                                                                        src="{{ asset('storage/app/public/uploads/staff-images/') . '/' . $train->image }}">
                                                                </a>
                                                            </li>
                                                                        ...
                                                                        <div class="avatar-group">
                                                                            ...
                                                                            @foreach ($res as $row)
                                                                                <a class="avatar avatar-xs" href="#">
                                                                                    <img alt=""
                                                                                        src="{{ asset('storage/app/public/uploads/staff-images/') . '/' . $row->image ?? '' }}"
                                                                                        title="{{ $row->name }}">
                                                                                </a>
                                                                            @endforeach
                                                                        </div>
                                                    ...
                                                    <td>{{ date('d M Y', strtotime($train->from)) }} -
                                                        {{ date('d M Y', strtotime($train->to)) }}</td>
                                                    <td>{{ $train->desc }}</td>
                                                    <td>{{ $train->cost }}</td>
                                                    ...
                                                    <td class="text-right">
                                                        <div class="dropdown dropdown-action">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                <a href="#" class="dropdown-item btn_edit_training"
                                                                    data="{{ $train->id }}" id="btn_edit_clients"><i
                                                                        class="la la-pencil" style="font-size: 20px;"></i></a>
                                                                <a href="#" class="dropdown-item btn_delete_training"
                                                                    data="{{ $train->id }}" id="btn_delete_clients"><i
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="post" action="{{ url('trainings') }}" id="trainingForm">
                            @csrf
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select class="select" name="training_type" required>
                                            ...
                                            <option value="Resource learning and development">Resource learning and
                                                development</option>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <div class=""><input class="form-control " name="from" type="date"
                                                required></div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <div class=""><input class="form-control " name="to" type="date"
                                                required></div>
                                    </div>
                                </div>
                            ...
                            <center>
                                <img id="loader" class="center" height="60px" width="90px"
                                    src=public/assets/img/loader/loader.gif style="display: none" />
                            </center>
                            <div class="submit-section">
                                <button class="btn btn-primary submit-btn btn_save_tain" type="submit"
        ...
        $(document).ready(function() {
            ...
            $('#trainingForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    processData: false,
                    beforeSend: function() {
                        $('.btn_save_tain').text('Saving...');
                        $(".btn_save_tain").prop("disabled", true);
                    },
                    success: function(data) {
                        ...
                        if (data.success) {
                            ...
                            $('.btn_save_tain').text('Save');
                            $(".btn_save_tain").prop("disabled", false);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('select[name="training_type"]')
                            .append(
                                `<option value="Skill Improvement" ${'Skill Improvement' == data['training'].training_type ? 'selected' : ''}>Skill Improvement</option>` +
                        ...
                        $.each(data.tran, function(key, value) {
                            $('select[name="trainer_name"]')
                                .append(
                                    `<option value="${value.id}" ${value.id == data['training'].trainer ? 'selected' : ''}>${value.f_name}</option>`
                                )
                        ...
                        $.each(data.employee, function(key, emp) {
                            $('select[name="emp_id"]')
                                .append(
                                    `<option value="${emp.id}" ${emp.id == data['training'].emp_id ? 'selected' : ''}>${emp.name}</option>`
                                )
                        ...
                        $('select[name="status"]')
                            .append(
                                `<option value="Active" ${'Active' == data['training'].status ? 'selected' : ''}>Active</option>` +
